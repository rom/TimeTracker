...
| 014 exactness | Rounding boundary tables (exactly on the increment, one second either side, zero, negative guard); rate × duration half-away-from-zero; mixed-currency addition refused. Property tests hold the rules that a table cannot state: rounding never moves a duration by a whole increment, always lands on a multiple of one, never rounds the wrong way, and is idempotent for the increment - which it is not once a minimum is configured, and that is why rounding happens exactly once, from the raw duration, at one call site. Every tenth and hundredth of an hour from 0.01 to 24 is checked against the integer answer, because 7.7 hours is 27719.999… seconds and a version that truncated instead of rounding would bill an hour of work a second short. That property was written while `ParseDuration` still used a float; it is what made replacing the float safe, and it now covers the unit-aware parse in both hours and minutes. Money round-trips through its decimal form for the values that break naive formatters (a negative under one major unit, an amount of five minor units), and parses what people actually type: a comma for a point, ordinary and non-breaking spaces as thousands separators. Four source-level rules in `internal/repocheck` now hold the representation the arithmetic rests on: no struct field in `internal/domain` or `internal/store` is a float; `internal/store` does not mention `float64` or `float32` anywhere, stated as a bright line because nothing a persistence layer does needs a fraction; no value anywhere named for its unit - anything ending in `Minor`, `Seconds`, `Cents` or `Millis` - is a float; and nothing outside the stated exception parses one from text. There is no exception left: `domain.ParseDuration` used to be one, reading the decimal-hours shorthand people type ("1.5") through `strconv.ParseFloat`, and writing the exemption down made it obvious the same parse is one lot of 3600 plus five tenths of one. It is integer arithmetic now, the tree has no float parse at all, and the list stays for the next one - it fails on a name that no longer parses anything, which is what reported the change. Floats used for what floats are for - a PDF's coordinates, an icon's anti-aliasing, a progress bar's width, an `Accept-Language` quality value - are untouched, which is why the rules key on the name and the layer rather than banning the type. |
...
| Symptom | Cause |
|---|---|
| a CSV column headed `hours` imported 2 as two minutes | the cell was parsed with the rule written for the duration box on a form, where a bare number means minutes; the column heading was never consulted (`internal/service`) |
