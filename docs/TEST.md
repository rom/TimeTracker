...
| 014 exactness | Rounding boundary tables (exactly on the increment, one second either side, zero, negative guard); rate × duration half-away-from-zero; mixed-currency addition refused. **Not covered:** no source-level check that a persisted field or total is never `float64`. Money is minor units and durations are whole seconds throughout, which is visible in the types, but nothing fails the build if that changes. |
